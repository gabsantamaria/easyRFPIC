...
# SaveFields True on the SWEEP so the E-point report has data across the band.
try:
    oAna.InsertSweep("Setup1",
        ["NAME:Sweep1", "IsEnabled:=", True, "RangeType:=", "LinearCount",
         ...
         "Type:=", "Interpolating", "SaveFields:=", True, "SaveRadFields:=", False])
except Exception as e:
...
q2d_msg(0, "Q2D cross-section build complete")
`;
  return ascii(code);
}

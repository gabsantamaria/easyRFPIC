    ...
    if (c.kind === 'boolean') {
      ...
      if (mirrorT) {
        ...
        if (mirrorT.pivot === 'origin') {
          ...
          for (const p of mine) {
            const inst0 = expandTransforms([p], pv, solved)[0] // pool = PRE-flatten solved: flattened replicas keep the group tag with baked offsets and would shift the pivot-'group' centroid;
      ...
      mine.forEach((p, k) => {
        const inst = expandTransforms([p], pv, solved)[0];
